...
| **A message template that was never an f-string.** Two `break_tie()` calls in `_star_round()` pass `"{int_to_words(len(tie), flowery=False)}-way tie in ..."` with no `f` prefix, so the exception reads back the literal braces instead of "three-way". | `starvote` 2.1.6 (Larry Hastings) | Cosmetic — exception type, tie set and control flow are all correct — but `_star_round()` backs single-winner STAR *and* Bloc STAR, so it reaches any STAR tie that gets as far as a custom tiebreaker or `tiebreaker=None`, which is precisely when a caller logs or shows the message. The sibling call sites at `:1979` and `:2350` have the `f`, which is what makes it a typo and not a convention. Not filed | [bloc-star-bpml-fixtures](bloc-star-bpml-fixtures.md) |
| **A test case whose ballot table doesn't produce its own result.** The unbreakable-tie fixture states its ballots three times; the rendered table's third row reads `5,4,3` where the plain listing and the `[ballots]` block both say `4,5,3`. | BetterVoting BPML test-case doc, *Unbreakable Tie (bloc — two seats)* | Totals become **12/13/11 instead of 12/12/12**, so there is no tie at all — the engine elects B then A and a reader rebuilding the case from the table never reaches the three-way tie the document exists to demonstrate | [bloc-star-bpml-fixtures](bloc-star-bpml-fixtures.md) |
